"""
EMA Pullback Strong Candle Strategy - MT5 Live Signal Bot
============================================================
...
kèm ảnh chart + nút "Vào Lệnh" vào Telegram, và tự đặt lệnh MT5 khi bấm nút.
...
# ---- Danh sách symbol ----
# AUTO_ALL_SYMBOLS = True  -> tự lấy toàn bộ symbol có trên MT5 (theo bộ lọc SYMBOL_FILTER bên dưới)
# AUTO_ALL_SYMBOLS = False -> dùng danh sách tay trong SYMBOLS_MANUAL
AUTO_ALL_SYMBOLS = True
SYMBOLS_MANUAL = ["EURUSD", "GBPUSD", "XAUUSD"]

# Lọc symbol khi AUTO_ALL_SYMBOLS = True. Dùng path chứa từ khoá (không phân biệt hoa/thường).
# Ví dụ path Exness thường dạng "Forex\\Majors\\EURUSDm", "Crypto\\BTCUSDm", "Metals\\XAUUSDm"...
# Để trống [] = không lọc theo path (áp dụng bộ lọc theo tiền tệ bên dưới).
SYMBOL_FILTER_KEYWORDS = []   # ví dụ: ["forex"] hoặc ["forex", "metals"]

# ---- Chỉ lấy các cặp được tạo từ danh sách tiền tệ này (mọi tổ hợp chéo) ----
# True  -> chỉ giữ symbol có 6 ký tự đầu là 2 mã tiền tệ nằm trong MAJOR_CURRENCIES (vd EURUSD, GBPJPY, AUDNZD...)
# False -> không áp dụng bộ lọc này (dùng SYMBOL_FILTER_KEYWORDS hoặc lấy hết)
FILTER_MAJOR_CURRENCIES_ONLY = True
MAJOR_CURRENCIES = {"AUD", "CAD", "CHF", "EUR", "GBP", "JPY", "NZD", "USD"}

# ---- Lấy thêm các symbol có mã gốc (base) nằm trong danh sách này, dù không phải "tiền tệ chính" ----
# Ví dụ: BTCUSDm, ETHUSDm, XAUUSDm (vàng), XAGUSDm (bạc), XPTUSDm (bạch kim), XPDUSDm (palladium)
# Để trống set() để không lấy.
EXTRA_BASE_SYMBOLS = {"BTC", "ETH", "XAU", "XAG", "XPT", "XPD"}

# ---- Lấy thêm nhóm Chỉ số (Indices) và Cổ phiếu (Stocks) theo path trong MT5 ----
# Path Exness dạng "Pro\\Indices\\US30", "Pro\\Stocks\\AAPL"... nên lọc theo từ khoá trong path.
INCLUDE_INDICES = True
INDEX_PATH_KEYWORDS = ["indices"]

INCLUDE_STOCKS = True
STOCK_PATH_KEYWORDS = ["stocks"]

# Khung thời gian riêng cho từng nhóm tài sản (key phải khớp với TIMEFRAMES bên dưới)
FX_METALS_CRYPTO_TIMEFRAMES = ["H4", "D1"]   # forex + BTC/ETH + kim loại: chạy đủ khung
INDEX_TIMEFRAMES = ["H4", "D1"]              # chỉ số: chạy đủ khung
STOCK_TIMEFRAMES = ["D1"]                    # cổ phiếu: CHỈ chạy Daily
...
def _match_currency_pair(symbol_name: str, base_set: set, quote_set: set) -> bool:
    """Khớp 6 ký tự đầu thành base(3)+quote(3), cả hai đều nằm trong tập cho phép.
    Quan trọng: nếu ký tự ngay sau 6 chữ cái đó vẫn là chữ HOA (vd chữ 'T' trong BTCUSDT),
    nghĩa là đây là một mã khác dài hơn (BTC/USDT) chứ không phải base/quote ta cần —
    phải loại, tránh khớp nhầm BTCUSDT thành BTC+USD."""
    m = re.match(r'^([A-Za-z]{6})', symbol_name)
    if not m:
        return False
    core = m.group(1).upper()
    base, quote = core[:3], core[3:]
    if base not in base_set or quote not in quote_set:
        return False
    rest = symbol_name[6:]
    if rest and rest[0].isalpha() and rest[0].isupper():
        return False   # ký tự HOA tiếp theo -> là 1 phần của mã khác (vd chữ T trong USDT)
    return True


def is_major_pair(symbol_name: str) -> bool:
    """True nếu symbol là 1 cặp tiền tệ hợp lệ trong MAJOR_CURRENCIES, vd EURUSDm, GBPJPYm..."""
    m = re.match(r'^([A-Za-z]{6})', symbol_name)
    base_ok = _match_currency_pair(symbol_name, MAJOR_CURRENCIES, MAJOR_CURRENCIES)
    if not base_ok:
        return False
    core = m.group(1).upper()
...
def is_extra_instrument(symbol_name: str) -> bool:
    """True nếu symbol là base thuộc EXTRA_BASE_SYMBOLS (BTC, ETH, XAU...) ghép với quote
    thuộc MAJOR_CURRENCIES (chỉ nhận USD/EUR/GBP...), vd BTCUSDm, ETHUSDm, XAUUSDm hợp lệ;
    BTCUSDTm (quote thực chất là USDT) hoặc ETHBTCm (quote là BTC, không có trong danh sách
    tiền tệ) sẽ bị loại."""
    return _match_currency_pair(symbol_name, EXTRA_BASE_SYMBOLS, MAJOR_CURRENCIES)


def get_symbols_to_watch():
    """Trả về dict {symbol_name: [danh sách tên khung áp dụng]}.
    Nếu AUTO_ALL_SYMBOLS=False, dùng SYMBOLS_MANUAL với toàn bộ TIMEFRAMES."""
    if not AUTO_ALL_SYMBOLS:
        return {s: list(TIMEFRAMES.keys()) for s in SYMBOLS_MANUAL}

    with mt5_lock:
        all_symbols = mt5.symbols_get()
    if all_symbols is None:
        log.error("Không lấy được danh sách symbol từ MT5.")
        return {}
    ...
    for s in all_symbols:
        ...
        if SYMBOL_FILTER_KEYWORDS and not any(kw.lower() in path_lower for kw in SYMBOL_FILTER_KEYWORDS):
            continue

        is_fx_extra = FILTER_MAJOR_CURRENCIES_ONLY and (is_major_pair(s.name) or is_extra_instrument(s.name))
        is_index = INCLUDE_INDICES and any(kw in path_lower for kw in INDEX_PATH_KEYWORDS)
        is_stock = INCLUDE_STOCKS and any(kw in path_lower for kw in STOCK_PATH_KEYWORDS)

        if not (is_fx_extra or is_index or is_stock):
            continue

        # nếu 1 symbol khớp nhiều nhóm (hiếm), ưu tiên fx_extra > index > stock cho khung thời gian
        if is_fx_extra:
            result[s.name] = FX_METALS_CRYPTO_TIMEFRAMES
        elif is_index:
            result[s.name] = INDEX_TIMEFRAMES
        elif is_stock:
            result[s.name] = STOCK_TIMEFRAMES

    log.info(f"Tự động lấy được {len(result)} symbol từ MT5.")
    return result
